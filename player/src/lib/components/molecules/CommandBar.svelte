<script lang="ts">
  ...
  const COMMAND_SUGGESTIONS = [
    ...
    },
    {
      prefix: ":title",
      label: ":title <name>",
      desc: "Filter library strictly by title",
    },
    {
      prefix: ":artist",
      label: ":artist <name>",
      desc: "Filter library strictly by artist",
    },
    {
      prefix: ":album",
      label: ":album <name>",
      desc: "Filter library strictly by album",
    },
    {
      prefix: ":mood",
      label: ":mood <name>",
      desc: "Filter library strictly by mood",
    },
    {
      prefix: ":tag",
      label: ":tag <name>",
  ...
  let matchingSuggestions = $derived.by(() => {
    ...
    if (val.startsWith("/") || val.startsWith(":")) {
      return COMMAND_SUGGESTIONS.filter((s) =>
        s.prefix.startsWith(val.split(" ")[0])
      );
    }
    return [];
  });
    ...
    if (matchingSuggestions.length > 0) {
      if (e.key === "ArrowDown") {
        ...
        selectedSuggestionIndex =
          (selectedSuggestionIndex + 1) % matchingSuggestions.length;
        return;
      }
      if (e.key === "ArrowUp") {
        e.preventDefault();
...
<div class={cn("relative w-full max-w-xl", className)}>
  <div class="relative flex items-center">
    ...
    <div
      class="absolute left-3 z-10 flex items-center pointer-events-none text-muted-foreground"
    >
