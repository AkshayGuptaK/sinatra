...
<script lang="ts">
	...
	function handleMuteToggle() {
		...
		if (isMuted) {
			...
			const targetVolume = rememberedVolume && rememberedVolume > 0 ? rememberedVolume : 0.7;
...
<div class={cn('flex items-center gap-2 select-none', className)}>
	...
	<ToggleButton
		...
		class={cn(
			'transition-colors',
			isMuted ? 'text-destructive hover:text-destructive' : 'text-muted-foreground hover:text-foreground'
		)}
