...
<div class={cn('flex items-center gap-1.5 flex-wrap min-w-0', className)}>
	...
	{#if isAdding}
		<input
			...
			placeholder="new tag"
			class="h-5 w-20 px-2 py-0 text-xs rounded-full border border-primary/50 bg-background text-foreground focus:outline-hidden focus:ring-1 focus:ring-primary/40 lowercase"
		/>
	{:else}
		<button
			type="button"
